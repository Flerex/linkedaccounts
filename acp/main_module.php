...
class main_module
{
	...
	{
		add_form_key('flerex_linkedaccounts_ucp_overview');
	...
	private function mode_management()
	{
		...
		if ($this->request->is_set_post('manage'))
		{
			if (!check_form_key('flerex_linkedaccounts_ucp_management'))
			{
				trigger_error('FORM_INVALID');
			...
			$user = $this->request->variable('user', null);
		...
		$this->template->assign_vars(array(
			...
			'U_FIND_USERNAME'	=> append_sid($this->phpbb_root_path . 'memberlist.' . $this->phpEx, 'mode=searchuser&amp;form=select_user&amp;field=username&amp;select_single=true'),
		));
	}
